...
// ── Canvas text wrap: 幅に収まるよう文字単位で折り返す ──────────────────────
...
// ── Canvas-based story image generation (pixel-perfect, no html2canvas) ──
async function generateStoryCanvas(
  ...
  const s = W / PREVIEW_WIDTH; // 4×

  // Mirror StoryCard sizing at full export resolution
  ...
  // Load wordcloud image from base64
  ...
  // Card height mirroring StoryCard layout（動的タイトル高さ対応）
  const cardH =
    imgPad                                              // image top margin
    + imgSize                                           // image (square)
    + Math.round(pw * 0.045 * 0.6) * s                // image bottom margin
    + Math.round(pw * 0.055 * 0.3) * s                // text area top pad
    + titleH                                            // title（動的）
    + Math.round(pw * 0.055 * 0.55) * s               // stats top gap
    + 3 * (Math.round(pw * 0.055 * 0.1) * s + statSz * 1.6)  // 3 stat lines
    + Math.round(pw * 0.055 * 0.7) * s                // footer gap
    + footSz * 2                                        // footer
    + txtPad;                                           // bottom pad
  ...
  // 2. Card base (glass gradient, clipped to rounded rect)
  ctx.save();
  ...
  // Wordcloud image (clipped to inner rounded rect)
  ctx.save();
  ...
  // y を 1行分の先端に揃える（最後の行の descender 分を除く）
  y -= titleSz * 0.25;
  ...
  // Footer (weight 500, letter-spacing)
  y += Math.round(pw * 0.055 * 0.7) * s + footSz;
  ctx.font = `500 ${footSz}px ${ff}`;
  ctx.fillStyle = "#999999";
  // letterSpacing は Chromium 94+ / Safari 16.4+ 対応
...
/* ── Story Card — polaroid style (rendered inside 9:16 preview & exported via html2canvas) ── */
function StoryCard({
  data,
  containerWidth,
}: {
  ...
  const imgPad = Math.round(containerWidth * 0.045); // polaroid margin around image
  ...
  const ff =
    "var(--font-inter), -apple-system, BlinkMacSystemFont, 'Hiragino Sans', sans-serif";
  ...
  return (
    ...
    >
      {/* ── Glass sheen: 上から下へ消えるハイライト ── */}
      <div
        style={{
          position: "absolute",
          inset: 0,
          background:
            "linear-gradient(180deg, rgba(255,255,255,0.42) 0%, rgba(255,255,255,0.08) 32%, rgba(255,255,255,0) 60%)",
          pointerEvents: "none",
          zIndex: 2,
          borderRadius: radius,
        }}
      ...
      <div
        style={{
          ...
          inset: 0,
          background:
            "linear-gradient(0deg, rgba(0,0,0,0.07) 0%, rgba(0,0,0,0) 30%)",
          pointerEvents: "none",
          zIndex: 2,
          borderRadius: radius,
        }}
      ...
      {/* Wordcloud with polaroid margin */}
      <div
        style={{
          position: "relative",
          zIndex: 3,
          padding: `${imgPad}px ${imgPad}px ${Math.round(imgPad * 0.6)}px`,
        }}
      >
        ...
        <img
          ...
          style={{
            width: "100%",
            display: "block",
            borderRadius: Math.round(containerWidth * 0.025),
            /* 画像にも薄いガラスっぽい影 */
      ...
      <div
        style={{
          position: "relative",
          zIndex: 3,
          padding: `${Math.round(textPad * 0.3)}px ${textPad}px ${textPad}px`,
        }}
      >
        {/* Title — stats より少し太め（600）*/}
        <p
          style={{
            margin: 0,
            fontSize: titleSize,
        ...
        <div style={{ marginTop: Math.round(textPad * 0.55) }}>
          {[
            { label: "通話時間", value: data.callTime },
            { label: "写真", value: `${data.photos}枚` },
            { label: "スタンプ", value: `${data.stamps}個` },
          ].map((stat) => (
            <p
              key={stat.label}
              style={{
                margin: `${Math.round(textPad * 0.1)}px 0 0`,
                fontSize: statSize,
                fontWeight: 400,
                color: "#3A3A3A",
        ...
        <p
          style={{
            margin: `${Math.round(textPad * 0.7)}px 0 0`,
            fontSize: footerSize,
            fontWeight: 500,
            color: "#999",
...
function StepLabel({ step, label }: { step: string; label: string }) {
  return (
    <div className="flex items-center gap-2 mb-4">
      <span className="flex-shrink-0 w-6 h-6 rounded-full bg-white/10 flex items-center justify-center text-[10px] font-bold text-zinc-400 tracking-wide">
        {step}
      </span>
...
function GlassCard({
  ...
  return (
    <div
      className={cn(
        "rounded-2xl bg-[#111111] border border-white/[0.07] overflow-hidden",
        className
      )}
    >
      {children}
...
export default function Home() {
      ...
      if (!res.ok) {
        const msg =
          typeof json.detail === "string"
            ? json.detail
            : json.detail?.[0]?.msg ?? res.statusText;
  ...
  /** ストーリーに投稿: Web Share API → 未対応・キャンセル時はダウンロード */
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white selection:bg-violet-500/30">
      {/* ── Header ── */}
      <header className="sticky top-0 z-50 bg-[#0A0A0A]/80 backdrop-blur-2xl border-b border-white/[0.05]">
        <div className="max-w-lg mx-auto flex items-center justify-between px-5 h-14">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-violet-500 to-pink-500 flex items-center justify-center shadow-lg shadow-violet-500/30">
          ...
          {cardData && (
            <button
              onClick={handleReset}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/5 hover:bg-white/10 transition-colors text-xs text-zinc-400 hover:text-white"
            >
      ...
      <main className="max-w-lg mx-auto px-5 pt-8 pb-24 space-y-7">
        ...
        {!cardData && !loading && (
          <div className="pt-4 pb-2">
            ...
            <h1 className="text-[2.6rem] font-black leading-[1.05] tracking-tighter">
        ...
        <section>
            ...
            <div className="flex items-center gap-4 p-5">
              <div
                className={cn(
                  ...
                  file ? "bg-green-500/20" : "bg-white/5 group-hover:bg-white/10"
                )}
              >
                {file ? (
              ...
              <div className="min-w-0 flex-1">
                {file ? (
                  <>
                    <p className="text-sm font-semibold text-green-400 truncate leading-snug">
                      {file.name}
                    </p>
          ...
          <button
            ...
            className={cn(
              "mt-3 w-full h-[54px] rounded-2xl font-bold text-[15px]",
        ...
        {loading && (
          <GlassCard className="animate-slide-up">
            <div className="p-5 space-y-4">
              <div className="h-2.5 w-28 rounded-full bg-white/5 skeleton" />
              <div className="aspect-square w-full rounded-xl bg-white/[0.04] skeleton" />
              <div className="space-y-2">
                <div className="h-2.5 w-full rounded-full bg-white/5 skeleton" />
                <div className="h-2.5 w-3/4 rounded-full bg-white/5 skeleton" />
                <div className="h-2.5 w-1/2 rounded-full bg-white/5 skeleton" />
              </div>
            </div>
          </GlassCard>
        )}
              ...
              <div className="p-5 flex flex-col items-center gap-5">
                ...
                <div className="w-full flex items-stretch gap-3">
                  <button
                    ...
                    disabled={shimmering || !showStoryPreview}
                    className={cn(
                      "flex-1 h-[54px] rounded-2xl font-bold text-[15px]",
                      "flex items-center justify-center gap-2.5",
                      "transition-all duration-300 active:scale-[0.97]",
                  ...
                  <div className="flex flex-col items-center gap-1.5">
                    <button
                      ...
                      className={cn(
                        ...
                        "transition-all duration-300 active:scale-[0.97] hover:bg-white/5",
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        backgroundColor === c.hex
                          ? "ring-2 ring-white ring-offset-2 ring-offset-[#111111] scale-[0.92]"
